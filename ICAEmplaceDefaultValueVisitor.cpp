...
ICAEmplaceDefaultValueVisitor::ICAEmplaceDefaultValueVisitor(clang::CompilerInstance & ci, const ICAConfig & config)
...
bool ICAEmplaceDefaultValueVisitor::VisitCXXMemberCallExpr(clang::CXXMemberCallExpr * expr)
{
...
void ICAEmplaceDefaultValueVisitor::clear()
{
    m_has_try_emplace.clear();
}

bool ICAEmplaceDefaultValueVisitor::hasTryEmplace(const clang::CXXRecordDecl * decl)
{
    if (!decl) {
        return false;
    }
...
void ICAEmplaceDefaultValueVisitor::makeReport(const clang::CXXMemberCallExpr * method_call)
{
