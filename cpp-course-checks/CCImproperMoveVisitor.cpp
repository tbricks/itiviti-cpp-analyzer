...
CCImproperMoveVisitor::CCImproperMoveVisitor(clang::CompilerInstance & ci, const ICAConfig & m_checks)
    : ICAVisitor(ci, m_checks)
{
    if (!isEnabled()) {
        return;
    }
...
bool CCImproperMoveVisitor::VisitFunctionDecl(clang::FunctionDecl * func_decl)
{
...
bool CCImproperMoveVisitor::VisitCallExpr(clang::CallExpr * call)
{
...
bool CCImproperMoveVisitor::dataTraverseStmtPre(clang::Stmt * stmt)
{
...
bool CCImproperMoveVisitor::dataTraverseStmtPost(clang::Stmt *stmt)
{
    if (auto imp_cast = clang::dyn_cast<clang::ImplicitCastExpr>(stmt)) {
        if (imp_cast->getCastKind() == clang::CK_NoOp) {
...
auto CCImproperMoveVisitor::findParameter(const clang::ParmVarDecl *parm) -> MaybeParamTraits
{
