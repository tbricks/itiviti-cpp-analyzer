...
struct DateTimeFactory
{
    ...
    DateTime create_from_string(const StringRef local_date) const // expected-note {{overload defined here}}
...
int main()
{
    ...
    f.create_from_string(s.c_str()); // expected-warning {{call to c_str can be removed, overload with types::StringRef available}}
}
