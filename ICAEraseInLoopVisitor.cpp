...
ICAEraseInLoopVisitor::ICAEraseInLoopVisitor(clang::CompilerInstance & ci, const ICAConfig & config)
...
bool ICAEraseInLoopVisitor::VisitForStmt(clang::ForStmt * for_stmt)
{
...
bool ICAEraseInLoopVisitor::dataTraverseStmtPre(clang::Stmt * s)
{
...
bool ICAEraseInLoopVisitor::dataTraverseStmtPost(clang::Stmt * s)
{
    m_parents.pop_back();
...
const clang::ForStmt * ICAEraseInLoopVisitor::findForStrmForVar(const clang::VarDecl * var_decl) const
{
    assert(var_decl);
...
void ICAEraseInLoopVisitor::reportInvalidErase(
...
const clang::Stmt * ICAEraseInLoopVisitor::getParentOfCurrentStmt() const
{
    if (m_parents.size() >= 2) {
...
bool ICAEraseInLoopVisitor::VisitCXXMemberCallExpr(clang::CXXMemberCallExpr * call_expr)
{
...
void ICAEraseInLoopVisitor::clear()
{
    resetContext();
    m_stack.clear();
}
